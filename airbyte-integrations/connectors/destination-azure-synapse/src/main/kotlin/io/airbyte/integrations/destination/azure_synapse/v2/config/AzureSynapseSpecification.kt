...
@Singleton
@JsonSchemaTitle("Azure Synapse Analytics Destination Specification")
@SuppressFBWarnings("NP_NONNULL_RETURN_VIOLATION")
class AzureSynapseSpecification : ConfigurationSpecification(), LoadTypeSpecification {
    @get:JsonSchemaTitle("Host")
    @get:JsonPropertyDescription(
        "The fully qualified server name of the Azure Synapse dedicated SQL pool " +
            "(e.g., yourserver.sql.azuresynapse.net)."
    )
    @get:JsonProperty("host")
    @get:JsonSchemaInject(json = """{"examples":["yourserver.sql.azuresynapse.net"],"order":0}""")
    val host: String = ""

    @get:JsonSchemaTitle("Port")
    @get:JsonPropertyDescription("The port of the Azure Synapse dedicated SQL pool.")
    @get:JsonProperty("port")
    @get:JsonSchemaInject(json = """{"minimum":0,"maximum":65536,"examples":["1433"],"order":1}""")
    val port: Int = 1433

    @get:JsonSchemaTitle("DB Name")
    @get:JsonPropertyDescription("The name of the Azure Synapse dedicated SQL pool (database).")
    ...
    @get:JsonSchemaInject(json = """{"examples":["dbo"],"default":"dbo","order":3}""")
    val schema: String = "dbo"

    @get:JsonSchemaTitle("User")
    @get:JsonPropertyDescription(
        "The username which is used to access the database. " +
            "For Azure Synapse, this is typically in the format user@server_name."
    )
    @get:JsonProperty("user")
    @get:JsonSchemaInject(json = """{"examples":["fivetran@yourserver"],"order":4}""")
    val user: String = ""
    ...
    @get:JsonPropertyDescription(
        "The encryption method which is used to communicate with the database. " +
            "Azure Synapse requires encrypted connections."
    )
...
class EncryptedVerify(
    ...
    @get:JsonPropertyDescription(
        "Specifies the host name of the server. The value of this property must match the subject property of the certificate. " +
            "Default for Azure Synapse is *.sql.azuresynapse.net"
    )
    @get:JsonProperty("hostNameInCertificate")
    @get:JsonSchemaInject(json = """{"examples":["*.sql.azuresynapse.net"],"order":3}""")
...
@Singleton
class AzureSynapseSpecificationExtension : DestinationSpecificationExtension {
