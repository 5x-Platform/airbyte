...
import io.airbyte.integrations.destination.azure_synapse.v2.config.AzureSynapseIsNotConfiguredForCopyInto
import io.github.oshai.kotlinlogging.KotlinLogging
import io.micronaut.context.annotation.Requires
import jakarta.inject.Singleton

@SuppressFBWarnings("NP_NONNULL_PARAM_VIOLATION", "kotlin coroutines")
class AzureSynapseDirectLoader(
    config: AzureSynapseConfiguration,
    stateStore: StreamStateStore<AzureSynapseStreamState>,
    private val streamDescriptor: DestinationStream.Descriptor,
    private val batch: Int,
    private val parent: AzureSynapseDirectLoaderFactory
) : DirectLoader {
    ...
    private val state =
        (stateStore.get(streamDescriptor) as AzureSynapseDirectLoaderStreamState?)
...
@Singleton
@Requires(condition = AzureSynapseIsNotConfiguredForCopyInto::class)
class AzureSynapseDirectLoaderFactory(
    val config: AzureSynapseConfiguration,
    val stateStore: StreamStateStore<AzureSynapseStreamState>,
) : DirectLoaderFactory<AzureSynapseDirectLoader> {
    private val log = KotlinLogging.logger {}
    ...
    override fun create(
        ...
        part: Int
    ): AzureSynapseDirectLoader {
        log.info { "Creating query builder for batch $batch of stream $streamDescriptor" }

        return AzureSynapseDirectLoader(config, stateStore, streamDescriptor, batch++, this)
    }
}
